...
from dml import (CheckpointCallback, Edge, Graph, Node, TensorBoardCallback,
                 Trainer)
from dml.utils import create_grad_scaler, get_device, load_checkpoint, set_seed
...
def main():
    ...
    parser.add_argument("--teacher-model", default="resnet50", type=str)
    parser.add_argument("--student-model", default="resnet18", type=str)
    parser.add_argument("--teacher-checkpoint", required=True, type=str)
    parser.add_argument("--weight-disco", default=1.0, type=float)
    ...
    graph = Graph(
        [
            ...
            ),  # node 0: teacher (no eval_fn → skipped)
            Node(
                model=student,
                optimizer=optimizer,
                scheduler=scheduler,
