...
class Trainer:
    ...
    def fit(self, train_dataloader, val_dataloader, epochs: int) -> None:
        ...
        for epoch in range(1, epochs + 1):
            ...
            train_score_meters = [AverageMeter() for _ in self.graph]
            ...
            for image, label in train_dataloader:
                ...
                outputs = self.graph.forward_all(image, device_type)
                losses = self.graph.compute_losses(outputs, label)
            ...
            self.graph.step_schedulers()

            val_score_meters = [AverageMeter() for _ in self.graph]
            self.graph.eval_all()
            with torch.no_grad():
